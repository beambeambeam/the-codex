    ...
    const onAutoScrollStart = React.useCallback(
      (event?: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (triggerMode === "press") {
          const timer = window.setInterval(onClick ?? (() => undefined), 50);
    ...
    const eventHandlers = React.useMemo(() => {
      ...
      > = {
        press: {
          ...
          // Intentionally left blank to prevent default click behavior during auto-scroll
          onClick: () => {
            /* no-op */
          },
        },
        hover: {
          ...
          onPointerLeave: onAutoScrollStop,
          onClick: () => {
            // no-op to prevent default click behavior during auto-scroll
          },
        },
        click: {
          onClick,
